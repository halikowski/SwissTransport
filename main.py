...
# loading raw_occupancy_data
    ...
    access1_df = session.sql('select * from accessibility_1')
    access2_df = session.sql('select * from accessibility_2')
    access_full_df = access1_df.join(access2_df, access1_df['sloid']==access2_df['sloid'], join_type='outer').drop(access2_df['sloid'])
    access_full_df = access_full_df.withColumnRenamed(access_full_df.columns[0],'sloid')
    access_full_df.write.save_as_table('test.curated.accessibility', mode ='overwrite')

    # transforming transport_subtypes table

    trans_subtypes_df = session.sql('select * from transport_subtypes')
    ...
    trans_subtypes_df.write.save_as_table('test.curated.transport_subtypes', mode='overwrite')

    # transforming transport_types table

    trans_types_df = session.sql('select * from transport_types')
    ...
    trans_types_df.write.save_as_table('test.curated.transport_types', mode='overwrite')

    # transforming line_data table

    line_data_df = session.sql('select * from line_data')
    ...
    line_data_df.drop_duplicates()
    line_data_df.show(5)
    line_data_df.write.save_as_table('test.curated.line_data', mode='overwrite')

    # transforming operators table

    business_types_df = session.sql('select distinct business_type_id, business_type_de, business_type_fr, business_type_it from operators')
    business_types_df = business_types_df.filter(~(col('business_type_id').contains(lit(','))))
    business_types_df.write.save_as_table('test.curated.business_types', mode='overwrite')

    operators_df = session.sql('select sboid, said, abbreviation, company_name, description, status, business_type_id from operators')
    ...
    operators_df.write.save_as_table('test.curated.operators', mode='overwrite')

    # transforming stop table

    municipality_df = session.sql('select distinct municipality_id, municipality from stop_data')
    municipality_df.write.save_as_table('test.curated.municipality_data', mode='overwrite')

    stop_data_df = session.sql('select distinct stop_id, stop_short_num, stop_control_num, stop_name, stop_city as city, municipality_id, canton_id, coord_e, coord_n, altitude from stop_data')
    stop_data_df.write.save_as_table('test.curated.stop_data', mode='overwrite')
